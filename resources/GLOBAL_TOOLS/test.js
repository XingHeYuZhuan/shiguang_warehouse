// 西北工业大学(NWPU) 拾光课程表适配脚本（测试用）
// 适配系统：EAMS 教务系统（jwxt.nwpu.edu.cn）
// 适配范围：本科（BACHELOR_AND_ASSOCIATE）
// 维护者：用户7227
// 注意：此文件用于 Beta 测试，正式适配请使用 resources/NWPU/nwpu_01.js
...
async function runImportFlow() {
    try {
        ...
        if (!studentId) {
            await window.AndroidBridgePromise.showAlert("导入失败", "未能获取学生 ID，请确认您已登录教务系统。", "确定");
            return;
        }
        ...
        if (semesters.length === 0) {
            await window.AndroidBridgePromise.showAlert("导入失败", "未能获取学期列表，请刷新页面后重试。", "确定");
            return;
        }
